...
ROM_DIR = Path("/recalbox/share/roms/neogeo")
SCRIPT_DIR = Path(__file__).resolve().parent
DATA_DIR = SCRIPT_DIR / "neogeo-validator-data"
...
def read_configured_core(path):
    """Devuelve el override neogeo.core activo, si existe."""
    if not path.exists():
        return ""
    ...
    with path.open("r", encoding="utf-8", errors="replace") as handle:
        for raw in handle:
            ...
            if key.strip().lower() == "neogeo.core":
                return value.strip().strip('"\'').lower()

    return ""


def load_neogeo_system_cores(path):
    """Lee cores, formatos, listas y prioridades del sistema Neo Geo."""
    definitions = {}
    if not path.exists():
        return definitions
    ...
    system = root.find(".//system[@name='neogeo']")
    if system is None:
        return definitions
    ...
    for node in emulator.findall("core"):
        ...
        definitions[name] = {
            ...
            "extensions": {
                value.rstrip("!").lower()
                for value in node.get("extensions", "").split()
                if value.startswith(".")
            },
        }
...
def discover_cores(
    ...
    """Descubre los cores realmente declarados e instalados para Neo Geo."""
    definitions = load_neogeo_system_cores(system_list)
    discovered = {}

    for name, definition in definitions.items():
        library = library_dir / f"{name}_libretro.so"
        ...
        discovered[name] = {
            ...
            "extensions": set(definition.get("extensions", set())),
        }
    ...
    default_core = configured or min(
        discovered,
        key=lambda name: (discovered[name]["priority"], name),
    )
...
def extract_interesting_log(lines):
    ...
    interesting = (
        ...
        "not found",
        "is required",
        "[fbneo] romset found at",
        "[fbneo] using rom with known crc",
        "[fbneo] timing set to",
    )
...
def fbneo_output_is_success(text):
    low = text.lower()
    return (
        "[fbneo] romset found at" in low
        and "[fbneo] using rom with known crc" in low
        and "[fbneo] timing set to" in low
    )
...
def test_core(core_name, rom_path, config_path, timeout_seconds):
    ...
    try:
        while time.monotonic() < deadline:
            ...
            if analyzer == "fbneo":
                if fbneo_output_is_success(text):
                    status = "OK"
                    reason = "Carga del ROMset confirmada por FBNeo"
                    break

            elif analyzer == "mame2003":
                if (
        ...
        if any(token in low for token in failure_strings):
            ...
            if analyzer == "fbneo" and fbneo_output_is_success(text):
                status = "OK"
                reason = "Carga del ROMset confirmada por FBNeo"
            elif (
                analyzer == "mame2003"
...
def detect(args):
    ...
    if not CORE_PREFERENCE:
        sys.exit("ERROR: no se ha encontrado ningún core Neo Geo instalado.")
    ...
    supported_extensions = {
        extension
        for core in CORE_PREFERENCE
        for extension in CORES[core]["extensions"]
    }
    ...
    if not roms:
        print("No hay ROMS Neo Geo compatibles que analizar.")

    fd, config_name = tempfile.mkstemp(
        prefix="neogeo-validator-",
        suffix=".cfg",
        dir="/tmp",
    )
    os.close(fd)
    ...
    report = {
        ...
        "discovered_cores": {
            core: {
                ...
                "extensions": sorted(CORES[core]["extensions"]),
            }
            for core in CORE_PREFERENCE
        },
        "roms": [],
    ...
    print()
    print("Neo Geo Validator - DETECT")
    print("==========================")
    ...
    for core in CORE_PREFERENCE:
        if (
            (core == "fbneo" or core.startswith("mame"))
            and not CORES[core]["list"].exists()
        ):
    ...
    try:
        for number, rom in enumerate(roms, 1):
            ...
            # BIOS y otros archivos auxiliares sin entrada visible de juego
            # quedan protegidos automáticamente.
            if rom_key not in games:
                item["status"] = "PROTECTED"
                item["reason"] = (
                    "Archivo sin entrada en gamelist.xml; posible BIOS/dependencia"
                )
                ...
                for core in CORE_PREFERENCE:
                    if rom.suffix.lower() not in CORES[core]["extensions"]:
                        item["cores"][core] = {
                            "status": "NOT_APPLICABLE",
                            "reason": "El core no admite este formato",
                        }
                        continue

                    uses_machine_list = core == "fbneo" or core.startswith("mame")
                    if uses_machine_list and not CORES[core]["list"].exists():
                        applicable.append(core)
                        unknown = True
                        item["cores"][core] = {
                            "status": "UNKNOWN",
                            "reason": "No está disponible la lista de máquinas del core",
                        }
                        continue

                    if uses_machine_list and machine not in lists[core]:
                        item["cores"][core] = {
                            "status": "NOT_LISTED",
                ...
                if item["compatible_cores"]:
                    ...
                    item["status"] = "UNKNOWN"
                    item["reason"] = (
                        "Ningún core instalado admite o reconoce esta ROM"
                    )
...
def move_unknown_roms(report, dry_run=False):
    ...
    for item in unknown_items:
        ...
        if target.exists():
            ...
            # La ROM ya fue movida en una ejecución anterior.
    ...
    for number, item in enumerate(valid, 1):
        ...
        if existing:
            for index in existing:
                ...
                # Para el core predeterminado dejamos que Recalbox use su valor.
...
def quarantine_non_valid(args, progress_offset=0, progress_total=0):
    ...
    print()
    print("Neo Geo Validator - REPAIR / QUARANTINE")
    print("=====================================")
...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Validador y reparador de ROMsets Neo Geo para Recalbox."
        ),
        epilog=(
    ...
    parser.add_argument(
        "--rom-dir",
        default=None,
        help="Directorio de ROMs Neo Geo (default: /recalbox/share/roms/neogeo)",
    )
    parser.add_argument(
        "--data-dir",
        default=None,
    ...
    detect_parser = commands.add_parser(
        "detect",
        help="Analizar todas las ROMS compatibles y generar un informe",
    )
